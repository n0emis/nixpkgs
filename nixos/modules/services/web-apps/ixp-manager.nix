...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.ixp-manager-db-setup = mkIf cfg.createDatabaseLocally {
      description = "DB-Setup for IXP-Manager";
      before = [ "ixp-manager-setup.service" ];
      after = [ "mysql.service" ];
      wantedBy = [ "multi-user.target" ];
      restartTriggers = [ configFile cfg.environmentFile ];
      serviceConfig = {
        Type = "oneshot";
      ...
      script = ''
        ...
        echo "ALTER USER ixpmanager@localhost IDENTIFIED WITH caching_sha2_password BY '$DB_PASSWORD';" | ${pkgs.mysql80}/bin/mysql -u root
      '';
    };
